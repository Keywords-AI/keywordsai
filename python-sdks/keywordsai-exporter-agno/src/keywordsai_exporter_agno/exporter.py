"""Keywords AI Agno Exporter - Export Agno traces to Keywords AI tracing endpoint."""
import logging
import os
import uuid
...
class KeywordsAIAgnoExporter:
    ...
    def export(self, trace_or_spans: Any) -> List[Dict[str, Any]]:
        """Export trace or spans to Keywords AI."""
    ...
    def build_payload(self, trace_or_spans: Any) -> List[Dict[str, Any]]:
        ...
        for span in spans:
            payload = self._span_to_keywordsai(
                span=span,
    ...
    def _propagate_trace_output(self, payloads: List[Dict[str, Any]]) -> None:
        ...
        if trace_output is not None:
            for payload in payloads:
                if is_blank_value(value=payload.get("output")) and payload.get("log_type") in (
                    LOG_TYPE_WORKFLOW,
                    LOG_TYPE_TASK,
                    "agent",
                ):
    ...
    def _extract_trace_context(
        ...
        trace_metadata = as_dict(
            get_attr(trace_obj, "metadata", "attributes", "tags")
        ) or {}
    ...
    def _span_to_keywordsai(
        self,
        span: Any,
        trace_context: TraceContext,
        span_id_map: Optional[Dict[str, str]] = None,
    ) -> Optional[Dict[str, Any]]:
        """Convert span to Keywords AI payload format."""
        ...
        span_metadata = as_dict(
            get_attr(span, "metadata", "attributes", "tags", "data")
        ) or {}
        ...
        prompt_messages = to_prompt_messages(value=span_input) if span_input is not None else None
        completion_message = (
        ...
        payload = {
            ...
            "span_name": str(span_name)[:255] if span_name else None,
            "span_path": span_path,
    ...
    def _map_log_type(
        ...
        """Map span kind to Keywords AI log type."""
        if span_kind:
            kind_str = str(span_kind).lower()
            for key, value in SPAN_KIND_TO_LOG_TYPE_MAP.items():
                if key in kind_str:
                    return value
        if model:
    ...
    def _send(self, payloads: List[Dict[str, Any]]) -> None:
        """Send payloads to Keywords AI endpoint."""
        try:
