 ...
 *     import type { KeywordsAILogParams } from './types/logTypes.js';
 *     
 *     // Initialize the client
 *     const client = new LogAPI({ apiKey: "your-api-key" });
 *     
 *     // Create a new log
 *     const logData: KeywordsAILogParams = {
 *       model: "gpt-4",
 *       input: "Hello, world!",
 ...
 *     errors if called.
 */
export class LogAPI extends BaseAPI<KeywordsAIFullLogParams, LogList, KeywordsAILogParams, never> {
  /**
   ...
   *     ```
   */
  async create(createData: KeywordsAILogParams): Promise<KeywordsAIFullLogParams> {
    ...
    return validatedData as KeywordsAIFullLogParams;
  }
   ...
   */
  async createLog(createData: KeywordsAILogParams): Promise<{ message: string }> {
    const validatedData = this.validateInput(createData);
   ...
   *     ```
   */
  async get(resourceId: string): Promise<KeywordsAIFullLogParams> {
    const response = await this.client.get(`${LOG_GET_PATH}/${resourceId}`);
    return response as KeywordsAIFullLogParams;
  }
   ...
   */
  async update(resourceId: string, updateData: never): Promise<KeywordsAIFullLogParams> {
    throw new Error(
 ...
 *
 * @param apiKey - Keywords AI API key (optional, reads from KEYWORDSAI_API_KEY env var if not provided)
 * @param baseUrl - Base URL for the API (optional, reads from KEYWORDSAI_BASE_URL env var or uses default)
 *
